...
int create_in_list(t_token *token, t_ast **node)
{
    ...
    if (!new->file_name)
        return (err_printf("malloc error: t_file->file_name for in_list\n"), 1);
    new ->next = NULL;
...
int create_heredoc_list(t_token *token, t_ast **node)
{
    ...
    new->file_name= ft_strdup(token->str);
    if (!new->file_name)
        return (err_printf("malloc error: t_file->file_name for heredoc_list\n"), 1);
    new->next = NULL;
...
int create_output_append_list(int code, t_token *token, t_ast **node)
{
    ...
    new = malloc(sizeof(t_file));
    if(!new)
        return(err_printf("malloc error: t_file for output_list\n"), 1);
    new->flag = code;
    new->file_name= ft_strdup(token->str);
    if (!new->file_name)
        return (err_printf("malloc error: t_file->file_name for output_list\n"), 1);
    new->next = NULL;
	new -> fd = -2;
