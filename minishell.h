...
/*   Updated: 2024/07/23 19:59:52 by ryagoub          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int					create_output_append_list(int code, t_token *token,
						t_ast **node);
...
int					expand_wc_file(t_shell *shell, t_ast *node, char *pattern,
						int code);
...
int					match_dir_matched(t_wc *wc_info, char *next_slash,
						char *new_directory);
int					match_dir_while(char *dir, char *pattern, char *next_slash,
						t_wc *wc_info);
...
int					matched_to_arg(t_dconts *matched, t_list **new_args,
						t_list **end_args);
int					add_matched_to_arg(t_dconts *matched, t_list **arg,
						t_ast *node);
...
// here_doc.c
int					open_hd_file(t_file *current, char *uniq_file,
						t_shell *shell);
...
int					create_key_value(char *str, char **equal, char **key,
						char **value);
...
int					extract_key_value(char *str, char **equal, char **key,
						char **value);
int					check_value(char *equal, char *key, t_shell *shell,
						t_list *curr_arg);
